	...
	while ( have_posts() ) {
		...
		if ( comments_open() ) {
			...
			<section class="container clearfix mt-5">
				<div class="col-sm-12 col-md-12 col-lg-12">
					<h3 class="hidden-lg">Comments for <?php echo $prolix->prolix_post_title(); ?></h3>
					<?php
					comments_template();
		...
		<section class="container clearfix mt-5">
			<div class="tagcloud">
				<div class="tag-cloud">
					<h3>Tags for <?php echo $prolix->prolix_post_title(); ?></h3>
					<?php
					...
					if ( $posttags ) {
						foreach ( $posttags as $tag ) {
							// if($tag->count > 5){
							echo '<a class="label ';
							echo $prolix->prolix_get_random_solid_class( $tag->slug );
